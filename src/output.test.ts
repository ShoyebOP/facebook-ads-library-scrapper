...
describe('output.ts', () => {
    ...
    describe('createIncrementalSaver', () => {
        ...
        it('does NOT re-save when no new URLs added', async () => {
            ...
            // File content should be identical (no overwrite)
    ...
    describe('source verification', () => {
        ...
        it('src/output.ts uses mkdir from node:fs/promises', async () => {
            const src = require('node:fs').readFileSync(
                new URL('./output.ts', import.meta.url),
                'utf-8',
            );
            expect(src).toContain('mkdir(');
        });

        it('src/output.ts uses padStart for timestamp padding', async () => {
            const src = require('node:fs').readFileSync(
                new URL('./output.ts', import.meta.url),
                'utf-8',
            );

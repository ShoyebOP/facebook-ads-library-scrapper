...
expected: |
  Run `bun run src/index.ts "test query"`. A stealth browser should launch (cloakbrowser with humanize, human_preset, stealth_args) without crashing. Logs should show browser launch activity with proxy credential redaction if proxy is used.
...
result: issue
reported: "nothing happens, no output nothing it just finishes immediately"
severity: blocker
...
- truth: "Run `bun run src/index.ts <query>` and browser launches with stealth config"
  status: failed
  reason: "User reported: nothing happens, no output nothing it just finishes immediately"
  severity: blocker
  test: 1
  root_cause: "src/index.ts exports main() but has no CLI entry point — module loads and exits without calling main()"
  artifacts:
    - path: "src/index.ts"
      issue: "Missing CLI entry point at module bottom"
  missing:
    - "Add CLI argument parsing and main() invocation when run directly"
  debug_session: ""
